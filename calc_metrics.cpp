...
inline void get_state_at(const vector<double>& data, size_t r, size_t c, size_t s, 
    ...
    // Helper for SIOD (reading stored data)
...
// =========================================================
// 2. FRESH ISMOD/SIOD CALCULATION (New Methodology)
// =========================================================
// Fresh integration from t_snap (matching Python approach)
// SIOD: forward paths (Level 0 = initial cell at t_snap)
// FinSIOD: reversed paths (Level 0 = final cell)
// Returns timing breakdown: trajectory_seconds, insmod_seconds, finsmod_seconds
struct SmodTimingBreakdown {
    double trajectory_seconds = 0.0;
    double insmod_seconds = 0.0;
    double finsmod_seconds = 0.0;
};

SmodTimingBreakdown compute_fresh_insmod_finsmod_slice(
    DynamicalSystem* sys,
    int rows, int cols,
    int start_row,
    int nx_global, int ny_global,
    ...
    cout << "[INFO] Starting " << direction_label << " FRESH SIOD/FinSIOD (from t=" 
    ...
    // =========================================================
    // PHASE 2: SIOD Scoring (forward paths)
    // =========================================================
    ...
    timing_result.insmod_seconds = chrono::duration<double>(t_insmod_end - t_insmod_start).count();
    cout << "[TIMING] SIOD scoring: " << fixed << setprecision(2) 
         << timing_result.insmod_seconds << "s" << endl;

    // =========================================================
    // PHASE 3: FinSIOD Scoring (reversed paths)
    // =========================================================
    ...
    for (int r = 0; r < rows; ++r) {
        for (int c = 0; c < cols; ++c) {
            ...
            // Create reversed path for FinSIOD
    ...
    timing_result.finsmod_seconds = chrono::duration<double>(t_finsmod_end - t_finsmod_start).count();
    cout << "[TIMING] FinSIOD scoring: " << fixed << setprecision(2) 
         << timing_result.finsmod_seconds << "s" << endl;
    
    return timing_result;
}

// =========================================================
// 3. EXACT OLB HELPER (LB using SIOD instead of FTLE)
// =========================================================
// Computes SIOD at a specific time slice t, similar to how LB computes FTLE
double get_smod_at_time_slice(
    ...
    double y_min, double y_max,
    double dt,
    bool use_finsmod,  // true for FinSIOD (reversed), false for SIOD (forward)
...
void compute_exact_olb_slice(
    ...
    double dt,
    int N_slices,
    bool use_finsmod,  // true for FinSIOD-based OLB, false for SIOD-based
    ...
    string smod_type = use_finsmod ? "FinSIOD" : "SIOD";
        ...
        for (int r = 0; r < rows; ++r) {
            ...
            for (int c = 0; c < cols; ++c) {
                ...
                for (int n = 0; n <= N_slices; ++n) {
                    ...
                    // Forward SIOD: from t to t_total
                    ...
                    // DEBUG: Check if forward and backward SIOD are equal (they shouldn't always be)
...
// Compute SIOD from pre-computed discretized paths (cached version)
double compute_smod_from_cached_paths(
...
// =========================================================
// For autonomous systems, SIOD(x0, t1->t2) depends only on duration |t2-t1|.
// Therefore:
//   - SMOD_fwd at time slice t (from t to τ) = SIOD over duration (τ-t)
//   - SMOD_bwd at time slice t (from t to 0) = SIOD over duration t (reversed)
//
...
void compute_exact_olb_slice_cached_v2(
    ...
    string smod_type = use_finsmod ? "FinSIOD" : "SIOD";
    cout << "[INFO] Starting CACHED OLB v2 (" << smod_type << ", " << N_slices << " time slices)..." << endl;
        ...
        for (int r = 0; r < rows; ++r) {
            ...
            for (int c = 0; c < cols; ++c) {
                ...
                for (int n = 0; n <= N_slices && valid_pixel; ++n) {
                    double t = n * dt_slice;
                    double duration_fwd = t_total - t;  // Duration for forward SIOD
                    double duration_bwd = t;             // Duration for backward SIOD
                    
                    // Extract center path segments based on DURATION (not position along trajectory)
                    // For autonomous systems: SIOD over duration D starting from x0 
                    ...
                    // Compute SIOD for each neighbor and take best
                    double smod_bwd = 0.0;
                    double smod_fwd = 0.0;
                    ...
                    for (int i = 0; i < 4; ++i) {
                        ...
                        // Compute SIOD for this neighbor pair
                        if (!center_bwd_segment.empty() && !neighbor_bwd_segment.empty()) {
                            double score = compute_smod_from_cached_paths(
                                center_bwd_segment, neighbor_bwd_segment, use_finsmod
                            );
                            if (score > smod_bwd) smod_bwd = score;
                        }
                        
                        if (!center_fwd_segment.empty() && !neighbor_fwd_segment.empty()) {
                            double score = compute_smod_from_cached_paths(
                                center_fwd_segment, neighbor_fwd_segment, use_finsmod
                            );
                            if (score > smod_fwd) smod_fwd = score;
                        }
                    }
                    ...
                    // F × B
                    sum_olb_fxb += sqrt(smod_bwd_safe * smod_fwd_safe);
                    // F + B
                    sum_olb_fpb += (smod_fwd_safe + smod_bwd_safe);
                    // F - B
                    sum_olb_fmb += (smod_fwd_safe - smod_bwd_safe);
                }
...
// what OLB requires. OLB needs SIOD computed from the same initial point
...
void compute_exact_olb_slice_cached(
    ...
    string smod_type = use_finsmod ? "FinSIOD" : "SIOD";
    cout << "[INFO] Starting CACHED OLB Calculation (" << smod_type << ", " << N_slices << " time slices)..." << endl;
        ...
        for (int r = 0; r < rows; ++r) {
            ...
            for (int c = 0; c < cols; ++c) {
                ...
                for (int n = 0; n <= N_slices && valid_pixel; ++n) {
                    ...
                    // For backward SIOD (measuring "where did I come from"):
                    ...
                    // For forward SIOD (measuring "where will I go"):
            ...
            if (cr % report_interval == 0 || cr == rows) {
                #pragma omp critical
                {
                   ...
                   double pct = (double)cr / rows * 100.0;
                   double eta = (elapsed / cr) * (rows - cr);
                   cout << "[PROGRESS] Cached OLB Integral (" << smod_type << "): " << cr << "/" << rows 
                        << " (" << fixed << setprecision(1) << pct << "%) - ETA: " 
                        << format_duration(eta) << endl << flush;
                }
            }
...
struct TimingStats {
    ...
    // SIOD metrics - broken down by phase
    double smod_traj_forward_seconds = 0.0;   // Trajectory generation + discretization (forward)
    double smod_traj_backward_seconds = 0.0;  // Trajectory generation + discretization (backward)
    double insmod_forward_seconds = 0.0;      // SIOD scoring only (forward)
    double insmod_backward_seconds = 0.0;     // SIOD scoring only (backward)
    double finsmod_forward_seconds = 0.0;     // FinSIOD scoring only (forward)
    double finsmod_backward_seconds = 0.0;    // FinSIOD scoring only (backward)
    ...
    void print_summary() const {
        ...
        double total_smod_traj = smod_traj_forward_seconds + smod_traj_backward_seconds;
        double total_insmod = insmod_forward_seconds + insmod_backward_seconds;
        double total_finsmod = finsmod_forward_seconds + finsmod_backward_seconds;
        double total_smod = total_smod_traj + total_insmod + total_finsmod;
        
        double total_all = total_standard + total_smod;
        ...
        cout << "╠══════════════════════════════════════════════════════════════════════════════╣" << endl;
        cout << "║  SIOD METRICS (broken down by phase):                                        ║" << endl;
        if (smod_traj_forward_seconds > 0 || smod_traj_backward_seconds > 0) {
            cout << "║    Traj Gen: Fwd " << fixed << setprecision(2) << setw(7) << smod_traj_forward_seconds 
                 << "s | Bwd " << setw(7) << smod_traj_backward_seconds << "s | Total " 
                 << setw(7) << total_smod_traj << "s         ║" << endl;
        }
        if (insmod_forward_seconds > 0 || insmod_backward_seconds > 0) {
            cout << "║    SIOD:   Fwd " << fixed << setprecision(2) << setw(7) << insmod_forward_seconds 
                 << "s | Bwd " << setw(7) << insmod_backward_seconds << "s | Total " 
                 << setw(7) << total_insmod << "s         ║" << endl;
        }
        if (finsmod_forward_seconds > 0 || finsmod_backward_seconds > 0) {
            cout << "║    FinSIOD:  Fwd " << fixed << setprecision(2) << setw(7) << finsmod_forward_seconds 
                 << "s | Bwd " << setw(7) << finsmod_backward_seconds << "s | Total " 
                 << setw(7) << total_finsmod << "s         ║" << endl;
        }
        cout << "║    ─────────────────────────────────────────────────────────────────────     ║" << endl;
        cout << "║    SIOD Total (Traj + SIOD + FinSIOD):                 " 
             << setw(7) << total_smod << "s         ║" << endl;
        cout << "╠══════════════════════════════════════════════════════════════════════════════╣" << endl;
        cout << "║  COMPARISON:                                                                 ║" << endl;
        if (total_standard > 0 && total_smod > 0) {
            double ratio = total_smod / total_standard;
            cout << "║    SIOD / Standard = " << fixed << setprecision(2) << ratio << "x";
            if (ratio > 1) {
                cout << "  (SIOD is " << setprecision(1) << (ratio - 1) * 100 << "% slower)";
            } else if (ratio < 1) {
                cout << "  (SIOD is " << setprecision(1) << (1 - ratio) * 100 << "% faster)";
            } else {
                cout << "  (about equal)";
            }
            cout << string(22, ' ') << "║" << endl;
        }
        if (total_insmod > 0 && total_finsmod > 0) {
            double ratio_if = total_finsmod / total_insmod;
            cout << "║    FinSIOD / SIOD = " << fixed << setprecision(2) << ratio_if << "x";
            cout << string(44, ' ') << "║" << endl;
        }
        ...
        cout << "[TIMING_DATA]"
             ...
             << " SMOD_TRAJ_FWD=" << smod_traj_forward_seconds 
             << " SMOD_TRAJ_BWD=" << smod_traj_backward_seconds
             << " INSMOD_FWD=" << insmod_forward_seconds 
             << " INSMOD_BWD=" << insmod_backward_seconds
             << " FINSMOD_FWD=" << finsmod_forward_seconds 
             << " FINSMOD_BWD=" << finsmod_backward_seconds
             << " TOTAL_STANDARD=" << total_standard
             << " TOTAL_SMOD_TRAJ=" << total_smod_traj
             << " TOTAL_INSMOD=" << total_insmod
             << " TOTAL_FINSMOD=" << total_finsmod
             << " TOTAL_SMOD=" << total_smod
...
int main(int argc, char* argv[]) {
    ...
    for (int i = idx; i < argc; ++i) {
        ...
        } else if (arg == "--compute-siod-forward") {
            compute_insmod_forward = true;
        } else if (arg == "--compute-siod-backward") {
            compute_insmod_backward = true;
        } else if (arg == "--compute-finsiod-forward") {
            compute_finsmod_forward = true;
        } else if (arg == "--compute-finsiod-backward") {
        ...
        } else if (arg == "--compute-olb-siod") {
            compute_olb_insmod = true;
        } else if (arg == "--compute-olb-finsiod") {
    ...
    // --- 2. SIOD/FinSIOD (Fresh Integration from t_snap, matching Python approach) ---
    // Forward: integrate from t_snap to t_total
    if ((compute_insmod_forward || compute_finsmod_forward) && (t_total - t_snap) > 1e-5) {
        SmodTimingBreakdown smod_timing_fwd = compute_fresh_insmod_finsmod_slice(
            sys, rows, cols, start_row, nx, ny, x_min, x_max, y_min, y_max,
            t_snap, t_total, dt,
            fwd_insmod_score, fwd_insmod_type, fwd_finsmod_score, fwd_finsmod_type, "Forward", use_orthogonal);
        timing.smod_traj_forward_seconds += smod_timing_fwd.trajectory_seconds;
        timing.insmod_forward_seconds += smod_timing_fwd.insmod_seconds;
        timing.finsmod_forward_seconds += smod_timing_fwd.finsmod_seconds;
    }
    
    // Backward: integrate from t_snap to 0.0
    if ((compute_insmod_backward || compute_finsmod_backward) && t_snap > 1e-5) {
        SmodTimingBreakdown smod_timing_bwd = compute_fresh_insmod_finsmod_slice(
            sys, rows, cols, start_row, nx, ny, x_min, x_max, y_min, y_max,
            t_snap, 0.0, dt,
            bwd_insmod_score, bwd_insmod_type, bwd_finsmod_score, bwd_finsmod_type, "Backward", use_orthogonal);
        timing.smod_traj_backward_seconds += smod_timing_bwd.trajectory_seconds;
        timing.insmod_backward_seconds += smod_timing_bwd.insmod_seconds;
        timing.finsmod_backward_seconds += smod_timing_bwd.finsmod_seconds;
    }
    ...
    // OLB using SIOD or FinSIOD, computed using CACHED trajectories for major speedup!
